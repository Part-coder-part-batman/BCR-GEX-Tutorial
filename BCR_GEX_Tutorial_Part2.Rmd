...
You need the `_bcr_data.tsv` files produced at the end of Part 1
(`BCR_GEX_Tutorial_Part1.Rmd`). These are joined to the Seurat objects in
Step 4 of this part.
...
|------|-------------------|
| `P1_LN_bcr_data.tsv` | Part 1 output, at `P1_LN/Output/` |
| `P1_PT_bcr_data.tsv` | Part 1 output, at `P1_PT/Output/` |
| `sample_filtered_feature_bc_matrix.h5` | Cell Ranger GEX output, at `per_sample_outs/<sample>/count/` |
...
One pattern worth clarifying: `^IGHD` catches the IGHD diversity genes
...
The DotPlot is the primary annotation tool. Read it row by row and look for the
marker group that lights up. Pay attention to both dot size (percent expressing)
and color (average expression level). This is the first pass -- several clusters
will be immediately classifiable, while others will need additional evidence from
BCR metadata and DEG analysis.

Some clusters stand out immediately: cluster 5 has the clearest signal in the
dataset with TCL1A, IGHD, IGHM, and IL4R pointing unambiguously to naive B cells.
Clusters 3 and 9 both show strong JCHAIN, MZB1, and XBP1 -- plasma cell markers.
Clusters 4 and 6 express BCL6, MEF2B, and AICDA. Clusters 0, 1, 2, and 7 all
show a memory B cell presence -- CD27-positive, no GC or naive markers. Clusters
0 and 1 look very similar with no obvious distinguishing markers between them.
Cluster 2 lights up on FCRL4, FCRL5, and interferon-stimulated genes. Cluster 7
shows elevated CD83.
...
Clusters 0 and 1 are substantially mixed between LN and PT, consistent with
quiescent memory B cell populations found in both tissues. Cluster 5 is
predominantly LN-derived, as expected for naive B cells. Cluster 7 has a
partial LN contribution (~25%). All remaining clusters are predominantly or
exclusively PT-derived.
...
Cluster 5 is predominantly IgM-dominant, consistent with its naive identity.
Clusters 1 and 2 show mixed isotype composition. The remaining clusters are
...
SHM frequency adds a further layer of evidence. Cluster 5 is clearly the lowest
in the dataset with a near-zero median, consistent with its naive identity.
Cluster 2 is notably lower than the other memory clusters (median ~0.03 vs
~0.05-0.06 for clusters 0 and 1). Clusters 0, 1, 3, 4, 6, 7, 8, and 9 are
broadly similar in the ~0.05-0.06 range. Cluster 10 appears elevated but has
...
of ~3.8. Clusters 0, 1, 5, and 7 show near-zero clonality. Cluster 2 shows
moderate clonality (mean clone size ~2.0).
...
Clusters 6 and 8 are the clear proliferating populations: cluster 6 shows
near-100% S and G2M phase, and cluster 8 shows ~55% S and G2M.
...
Several clusters show interpretable signals: cluster 2 lights up on OASL, IFIT1,
FCRL4, and USP18. Cluster 3 shows immunoglobulin heavy and light chain V gene
transcripts alongside SDC1 and GAS6. Cluster 4 shows SERPINA9 and MEF2B. Cluster
6 is dominated by mitotic genes. Cluster 7 shows NR4A1, NR4A2, EGR2, and EGR1.
Cluster 8 shows S-phase histone genes. Cluster 9 shows NKX6-3. Clusters 0 and 1
show weak signals from this view alone and are resolved in section 7.12.
...
Clusters 0 and 1 cannot be distinguished from the DotPlot or heatmap alone --
both show a memory B cell profile. We run each against the other two combined.
Cluster 2 is included as a reference point.

```{r deg-cluster-0-1-2}
deg_0_vs_1_2 <- FindMarkers(obj, ident.1 = 0, ident.2 = c(1, 2),
                             min.pct = 0.25, logfc.threshold = 0.1,
                             only.pos = FALSE, verbose = FALSE)
deg_1_vs_0_2 <- FindMarkers(obj, ident.1 = 1, ident.2 = c(0, 2),
                             min.pct = 0.25, logfc.threshold = 0.1,
                             only.pos = FALSE, verbose = FALSE)
deg_2_vs_0_1 <- FindMarkers(obj, ident.1 = 2, ident.2 = c(0, 1),
                             min.pct = 0.25, logfc.threshold = 0.1,
                             only.pos = FALSE, verbose = FALSE)

for (res in list(list(deg_0_vs_1_2, "Cluster 0 vs 1+2"),
                 list(deg_1_vs_0_2, "Cluster 1 vs 0+2"),
                 list(deg_2_vs_0_1, "Cluster 2 vs 0+1"))) {
  df      <- res[[1]]
  label   <- res[[2]]
  df$gene <- rownames(df)
...
**Cluster 0** lacks strong functional identity markers. The top DEGs are
ribosomal protein genes, translation factors, and lncRNAs with modest fold
changes and small differences in percent expressing. Combined with near-zero
clonality and mixed isotype composition, we label this cluster **MBC**.

**Cluster 1** shows CR1, CR2, NOTCH2, BACH2, EBF1, AKT3, FOXP1, and KLF12 as
its top specific markers. We name this cluster **MBC_CR2pos**.

**Cluster 2** has the clearest pairwise signal: OASL, USP18, FCRL4, IFIT1,
ISG15, DUSP4, RSAD2, CCR1, IFIT3, and ITGAX, all with high fold changes (2.5-4.0).
We call this cluster **MBC_Atypical**.

## 7.13 Pairwise DEGs: confirming PC and GC identities

The PC and GC clusters already have provisional identities from the DotPlot --
this step confirms and locks them in.

```{r deg-pc-gc}
deg_3_vs_689 <- FindMarkers(obj, ident.1 = 3, ident.2 = c(6, 8, 9),
                             min.pct = 0.20, logfc.threshold = 0.1,
                             only.pos = TRUE, verbose = FALSE)
deg_6_vs_389 <- FindMarkers(obj, ident.1 = 6, ident.2 = c(3, 8, 9),
                             min.pct = 0.20, logfc.threshold = 0.1,
                             only.pos = TRUE, verbose = FALSE)
deg_8_vs_369 <- FindMarkers(obj, ident.1 = 8, ident.2 = c(3, 6, 9),
                             min.pct = 0.20, logfc.threshold = 0.1,
                             only.pos = TRUE, verbose = FALSE)
deg_9_vs_368 <- FindMarkers(obj, ident.1 = 9, ident.2 = c(3, 6, 8),
                             min.pct = 0.20, logfc.threshold = 0.1,
                             only.pos = TRUE, verbose = FALSE)

for (res in list(list(deg_3_vs_689, "Cluster 3 vs 6+8+9"),
                 list(deg_6_vs_389, "Cluster 6 vs 3+8+9"),
                 list(deg_8_vs_369, "Cluster 8 vs 3+6+9"),
                 list(deg_9_vs_368, "Cluster 9 vs 3+6+8"))) {
  df      <- res[[1]]
  label   <- res[[2]]
  df$gene <- rownames(df)
  df      <- df %>% dplyr::arrange(desc(avg_log2FC))
...
**Cluster 3** is distinguished by immunoglobulin heavy and light chain expression
alongside GAS6, SPAG4, FNDC3B, ITM2C, and PECAM1. This is the main PC population.

**Cluster 6** is dominated by mitotic genes (KIF18B, KIFC1, CCNB2, CDCA7, MYBL1)
with very high fold changes, alongside BACH2 and SERPINA9 placing these cells in
a GC context. Combined with near-100% S+G2M composition, this is the cycling GC
B cell population: **GC_Cycling**.

**Cluster 8** shows S-phase histone genes alongside FCRL4, GBP1, IFI44L, USP18,
and TNFRSF1B. The combination of proliferation markers with FCRL4 and IFN genes
links this cluster to MBC_Atypical -- cycling atypical memory B cells:
**MBC_Cycling**.

**Cluster 9** shows NKX6-3, CYS1, RASSF6, VDR, CD9, and DENND6B, confirming a
PC identity distinct from cluster 3: **PC_2**.
...
|---------|---|-------|-------------|
| 0 | 1,793 | MBC | Near-zero clonality, mixed isotype, no specific identity markers |
| 1 | 1,507 | MBC_CR2pos | CR1, CR2, NOTCH2, BACH2, EBF1, AKT3 specific vs clusters 0 and 2 |
| 2 | 1,222 | MBC_Atypical | FCRL4, ITGAX, strong IFN program (OASL, USP18, ISG15, IFITs, DUSP4) |
| 3 | 872 | PC | IGHG1/2/3/4, IGKC, GAS6, SPAG4, FNDC3B, ITM2C, PECAM1 |
| 4 | 663 | GC | BCL6, MEF2B, SERPINA9 -- non-proliferating GC B cells |
| 5 | 653 | Naive | TCL1A, IGHD, IGHM, IL4R; near-zero clonality; LN-enriched |
| 6 | 504 | GC_Cycling | Pure mitotic program (KIF18B, KIFC1, CCNB2); near-100% S+G2M; PT-only |
| 7 | 406 | MBC_NR4A | NR4A1, NR4A2, EGR1, EGR2, CD69; partial LN contribution |
| 8 | 209 | MBC_Cycling | HIST1H3H/1E, FCRL4, GBP1, IFI44L, USP18 -- cycling atypical MBCs |
| 9 | 96 | PC_2 | NKX6-3, CYS1, RASSF6, VDR, CD9; PT-only; second PC population |
| 10 | 32 | Unsure | No interpretable signature at 32 cells; PT-only |
...
```{r annotated-dotplot}
cluster_order <- c(
  "Naive", "MBC", "MBC_CR2pos", "MBC_NR4A", "MBC_Atypical",
  "MBC_Cycling", "GC", "GC_Cycling", "PC", "PC_2", "Unsure"
)
...
**Memory B cell compartment (clusters 0, 1, 2, 7, 8):** MBC and MBC_CR2pos are
both present in LN and PT, both showing near-zero clonality and mixed isotype
composition. They differ in expression of complement receptors (CR1, CR2) and
associated transcription factors (NOTCH2, BACH2) in MBC_CR2pos. MBC_Atypical
co-expresses atypical memory B cell markers (FCRL4, ITGAX) with a strong
interferon-stimulated gene program. MBC_NR4A shows NR4A1, NR4A2, EGR1, EGR2,
and CD69 with a partial LN contribution. MBC_Cycling shares the FCRL4 and IFN
signature with MBC_Atypical but additionally shows S-phase proliferation markers.

**GC and PC compartment (clusters 3, 4, 6, 9):** The tumor site harbors an active
germinal center response. GC shows the BCL6/MEF2B/SERPINA9 signature without
proliferation. GC_Cycling is dominated by the mitotic gene program with
near-100% S+G2M phase. PC and PC_2 are the plasma cell output, both predominantly
PT-located, with PC_2 showing a distinct expression profile (NKX6-3, CYS1,
RASSF6, VDR, CD9) at 96 cells.

**Naive B cells (cluster 5):** TCL1A, IGHD, IGHM, IL4R with near-zero clonality
and IgM-dominant isotype. LN-enriched, as expected.

**Cluster 10 (Unsure):** 32 cells, PT-only, no interpretable B cell identity
signature at this cell count.

A note on resolution: this tutorial uses a single patient with one LN and one PT
sample. Some clusters -- particularly PC_2 (96 cells), MBC_Cycling (209 cells),
and Unsure (32 cells) -- have low cell numbers, which limits the confidence of
their annotation. A larger cohort would be needed to fully characterize these
populations. The annotations here are a guide and a starting point, not a
definitive biological conclusion.
